...
from maskrcnn_benchmark.engine.trainer import do_train, do_train_test
...
def train(cfg, local_rank, distributed, zero_shot, skip_optimizer_resume=False, save_config_path = None):
    ...
    # data_loader, data_loaders_val = prepare_dataset(cfg, data_loader, data_loaders_val, distributed, task_id)
    ...
    #     if 'visual_prompt' in name or 'textual_prompt' in name:
    ...
    # scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=cfg.)
    if distributed:
    ...
    # logger = logging.getLogger('final_res')
    # if is_main_process():
    #     for name, p in model.named_parameters():
    #         if p.requires_grad:
    ...
    if cfg.DATASETS.ALTERNATIVE_TRAINING:
        ...
        for tid in range(t_num):
            ...
            for name, p in model.named_parameters():
                p.requires_grad_(False)
                if cfg.LPAI.VISUAL_PROMPT or cfg.LPAI.TEXTUAL_PROMPT:
                    if cfg.LPAI.INTERACT_TYPE == 'maple' or cfg.LPAI.INTERACT_TYPE == 'linear':
                        if 'prompts' + '.' +str(tid)+ '.' in name:
                            p.requires_grad_(True)
                            # logger.info(f"{name} : Not Frozen, param number, {p.numel()}")
                            print(f"{name} : Not Frozen, param number, {p.numel()}")
                        if 'visual_prompt' + '.' + str(tid) + '.' in name or 'textual_prompt' + '.' + str(
                                tid) + '.' in name:
                            p.requires_grad_(True)
                            # logger.info(f"{name} : Not Frozen, param number, {p.numel()}")
                            print(f"{name} : Not Frozen, param number, {p.numel()}")
                    elif cfg.LPAI.INTERACT_TYPE == 'l2p':
                        if 'prompts' in name:
                            p.requires_grad_(True)
                            # logger.info(f"{name} : Not Frozen, param number, {p.numel()}")
                            print(f"{name} : Not Frozen, param number, {p.numel()}")
                        ...
                        if 'visual_prompt' + '.' + str(tid) + '.' in name or 'textual_prompt' + '.' + str(
                            ...
                            # logger.info(f"{name} : Not Frozen, param number, {p.numel()}")
                            print(f"{name} : Not Frozen, param number, {p.numel()}")
                if cfg.LPAI.INTERACT:
                    if 'interactModuleList' + '.' + str(tid) + '.' in name:
                        p.requires_grad_(True)
                        # logger.info(f"{name} : Not Frozen, param number, {p.numel()}")
                        print(f"{name} : Not Frozen, param number, {p.numel()}")
                # if cfg.LPAI.VISUAL_PROMPT or cfg.LPAI.TEXTUAL_PROMPT:
                #     if cfg.LPAI.INTERACT_TYPE == 'maple':
                #         if 'prompts' in name:
                #             p.requires_grad_(True)
                #             logger.info(f"{name} : Not Frozen, param number, {p.numel()}")
                #     else:
                ...
                #             logger.info(f"{name} : Not Frozen, param number, {p.numel()}")
                # if cfg.LPAI.INTERACT:
                #     if 'interactModuleList' in name:
                #         p.requires_grad_(True)
                #         logger.info(f"{name} : Not Frozen, param number, {p.numel()}")
            # scheduler = make_lr_scheduler(cfg, optimizer)
            if (not cfg.LPAI.VISUAL_PROMPT) and (not cfg.LPAI.TEXTUAL_PROMPT):
                optimizer = None
                scheduler = None
            else:
                optimizer = make_optimizer(cfg, model)
                scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=cfg.LPAI.EPOCH)
            ...
            #     scheduler.step()
            # exit(0)
            trainable_param_size, total_param_size = count_parameters(model)
            logger.info(f'trainable param size {trainable_param_size}, total size {total_param_size}')
            # if True:
            # # tid = tid+1
            #     for name, p in model.named_parameters():
            #         p.requires_grad_(False)
            #         if 'visual_prompt'+'.'+str(tid)+'.' in name or 'textual_prompt'+'.'+str(tid)+'.' in name:
            #             p.requires_grad_(True)
            #             logger.info(f"{name} : Not Frozen, param number, {p.numel()}")
            #         if 'interactModuleList' + '.' + str(tid) + '.' in name:
            #             p.requires_grad_(True)
            ...
            data_loader, data_loaders_val = prepare_dataset(cfg, distributed, tid)
            # model.task_id = tid
            if isinstance(model, nn.DataParallel):
                model.module.task_id = tid
            else:
                model.task_id = tid
            # model.module.task_id = tid
            # try:
            #     model.update_task_id(tid)
            # except:
            #     model.module.update_task_id(tid)
        ...
        with open(f'./FINAL_RES/res_{datetime.now().date()}-{datetime.now().hour}:{datetime.now().minute}:{datetime.now().second}.json', 'w') as f:
            json.dump(score_map, f)
        print(score_map)
        # final_res = {}
        # final_res['refcoco'] = [0,0,0]
        # for key in score_map:
        #     final_res['refcoco'][0] += score_map[key]['refcoco'][0]
        #     final_res['refcoco'][1] += score_map[key]['refcoco'][1]
        #     final_res['refcoco'][2] += score_map[key]['refcoco'][2]
        # final_res['refcoco'][0] /= t_num
        # final_res['refcoco'][1] /= t_num
        # final_res['refcoco'][2] /= t_num
        # p1 = final_res['refcoco'][0]
        # p5 = final_res['refcoco'][1]
        # p10 = final_res['refcoco'][2]
        # logger.info(f'refcoco: p@1: {p1}, p@5: {p5}, p@10: {p10}')

    return model


def prepare_dataset(cfg, distributed, task_id):
    logging.info(f"begin Task [{task_id} training.]")
    ...
    logging.info(f"training samples number: {len(data_loader.dataset)}")

    if cfg.TEST.DURING_TRAINING:
        dataloader_val_list = []
        for i in range(task_id, task_id+1):
            data_loaders_val = make_data_loader(cfg, is_train=False, is_distributed=distributed, task_id=[i])
            # data_loaders_val = data_loaders_val[0]
            dataloader_val_list.append(data_loaders_val)
        # data_loaders_val = data_loaders_val[0]
        #     logging.info(f"testing samples number: {len(data_loaders_val.dataset)}")
    else:
        data_loaders_val = None
    return data_loader, dataloader_val_list
...
def t(cfg, model, distributed, verbose=False):
    if distributed:
        model = model.module
...
def main():
    ...
    # parser.add_argument("--device_id", default=None, action='cuda id')
    ...
    for task_id, ft_cfg in enumerate(ft_configs, 1):
        for shuffle_seed in shuffle_seeds:
            ...
            if args.custom_shot_and_epoch_and_general_copy:
                ...
                cfg_.SOLVER.MAX_EPOCH = custom_epoch = 1
            ...
            if custom_shot == 10000:
                ...
                if not args.skip_test:
                    t(cfg_, model, args.distributed)
                
                if args.keep_testing:
                    # for manual testing
                    cfg_.defrost()
                    cfg_.DATASETS.TEST = ("test", )
                    t(cfg_, model, args.distributed, verbose=True)
