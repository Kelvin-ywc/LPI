...
def do_train(
    ...
    for epoch in range(cfg.LPAI.EPOCH):
                ...
                with autocast():
                    if len(captions) > 0:
                        if isinstance(model, nn.DataParallel):
                            assert model.module.task_id == task_id, f'model: {model.module.task_id}, task id: {task_id}'
                        else:
                            assert model.task_id == task_id, f'model: {model.task_id}, task id: {task_id}'
                        # assert model.module.task_id == task_id, f'model: {model.module.task_id}, task id: {task_id}'
    ...
    if val_data_loader:
        ...
        checkpointer.save(f"model_final_{task_id}", **arguments)
        if is_main_process():
            print("Evaluating")
        eval_result = 0.0
        model.eval()
        if cfg.SOLVER.TEST_WITH_INFERENCE:
            with torch.no_grad():
                try:
                    ...
                    _model = model
                # print(_model.interface_visual(task_id))
                # print(_model.interface_textual(task_id))
                # torch.save(_model.interface_visual(task_id), f'./prompt_save/visual_{task_id}.pth')
                # torch.save(_model.interface_textual(task_id), f'./prompt_save/textual_{task_id}.pth')
                # clustering
                if _model.cfg.LPAI.INTERACT_TYPE != 'l2p':
                    _model.clustering(data_loader, device=device)
                res_map = {}
                res_map['refcoco'] = {
                    'testA': {},
                    'testB': {},
                ...
                for idx in range(len(val_data_loader)):  # idx 任务id
                    for iidx in range(len(val_data_loader[idx])):
                        val_dataloader = val_data_loader[idx][iidx]
                        _result = {}
                        if len(val_dataloader.dataset) != 0:
                            # val_data_loader[i]: test_A, test_B, val dataloader
                            logger.info(
                                f'evaluating on {val_dataloader.dataset.dataset_name}, task type: {val_dataloader.dataset.task_name}, task_id: {idx}')
                            # visualizer = GLIPDemo(
                            #     cfg,
                            #     min_image_size=800,
                            ...
                            res_map[val_dataloader.dataset.dataset_name][val_dataloader.dataset.task_name][idx] = \
                                _result[val_dataloader.dataset.dataset_name]

                        # res_map[idx] = _result
                        print(
                            f'evaluate on task {val_dataloader.dataset.dataset_name}, {val_dataloader.dataset.task_name}, {iidx}, res: {_result}')
                # if is_main_process():
...
def do_train_org(
    ...
    for iteration, (images, targets, idxs, positive_map, positive_map_eval, greenlight_map) in enumerate(data_loader):
            ...
            with autocast():
                if len(captions) > 0:
                    if isinstance(model, nn.DataParallel):
                        ...
                        assert model.task_id == task_id, f'model: {model.task_id}, task id: {task_id}'
                    # assert model.module.task_id == task_id, f'model: {model.module.task_id}, task id: {task_id}'
                    loss_dict = model(images, targets, captions, positive_map, greenlight_map = greenlight_map, task_id=[task_id])
                else:
                    loss_dict = model(images, targets)
            losses = sum(loss for loss in loss_dict.values())
        ...
        if iteration % 2 == 0 or iteration == max_iter:
        # if iteration % 1 == 0 or iteration == max_iter:
            #logger.info(
            if global_rank <= 0:
                print(
        ...
        # if val_data_loader and (iteration % checkpoint_period == 0 or iteration == max_iter):
        if val_data_loader and iteration == max_iter:
                ...
                with torch.no_grad():
                    ...
                    for idx in range(len(val_data_loader)): # idx 任务id
                        for iidx in range(len(val_data_loader[idx])):
                            ...
                            if len(val_dataloader.dataset) != 0:
                            # val_data_loader[i]: test_A, test_B, val dataloader
                                logger.info(f'evaluating on {val_dataloader.dataset.dataset_name}, task type: {val_dataloader.dataset.task_name}, task_id: {idx}')
                                _result = inference(
                                    model = _model,
                                    data_loader = val_dataloader,
                                    dataset_name="val",
                                ...
                                res_map[val_dataloader.dataset.dataset_name][val_dataloader.dataset.task_name][idx] = _result[val_dataloader.dataset.dataset_name]

                            # res_map[idx] = _result
                            print(f'evaluate on task {val_dataloader.dataset.dataset_name}, {val_dataloader.dataset.task_name}, {iidx}, res: {_result}')
                    # if is_main_process():
                    #     eval_result = _result[0].results['bbox']['AP']
            else:
                results_dict = {}
...
def do_train_test(
    ...
    if cfg.SOLVER.TEST_WITH_INFERENCE:
        with torch.no_grad():
            ...
            print('********************************************************',len(_model.all_keys))
            if task_id == 11:
                torch.save(_model.all_keys,'./all_key/all_key.pth')
            for key in _model.all_keys:
                print(key.shape)
            return
            res_map = {}
            res_map['refcoco'] = {
            ...
            # for idx in range(len(val_data_loader)):  # idx 任务id
            #     for val_dataloader in val_data_loader[idx]:
            #         # val_data_loader[i]: test_A, test_B, val dataloader
            #         logger.info(
            #             f'evaluating on {val_dataloader.dataset.dataset_name}, task type: {val_dataloader.dataset.task_name}, task_id: {idx}')
            #         _result = inference(
            ...
            #             task_id=idx
            #         )
            #         res_map[val_dataloader.dataset.dataset_name][val_dataloader.dataset.task_name][idx] = _result[
            #             val_dataloader.dataset.dataset_name]

            #         # res_map[idx] = _result
            #         print(
            #             f'evaluate on task {val_dataloader.dataset.dataset_name}, {val_dataloader.dataset.task_name}, {idx}, res: {_result}')
            for idx in range(len(val_data_loader)):  # idx 任务id
                for iidx in range(len(val_data_loader[idx])):
                    val_dataloader = val_data_loader[idx][iidx]
                    _result = {}
