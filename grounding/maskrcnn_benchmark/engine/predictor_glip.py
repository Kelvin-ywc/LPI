...
from tools.utils.colors import ncolors
...
class GLIPDemo(object):
    ...
    def build_transform(self):
        ...
        transform = T.Compose(
            [
                ...
                T.Resize(min_size=min_size, max_size=max_size, restrict=True),
                T.ToTensor(),
                # to_bgr_transform,
                normalize_transform,
            ]
    ...
    def visualize_with_target(self,
            original_image,
            predictions,
            thresh=0.5,
            alpha=0.0,
    ...
    def visualize_with_predictions(self, 
        ...
        # result = self.overlay_entity_names(result, top_predictions, text_size=text_size, text_pixel=text_pixel, text_offset = text_offset, text_offset_original = text_offset_original)
        # TODO
        result = self.overlay_scores(result, top_predictions)
        if self.cfg.MODEL.MASK_ON:
            result = self.overlay_mask(result, top_predictions)
        return result, top_predictions

    def visualize_with_predictions_metric(self,
            original_image,
            predictions,
            thresh=0.5,
            alpha=0.0,
        ...
        if metric == 'R@1':
            ...
            k=10
        top_predictions = self._post_process_top_k(predictions, thresh, k)

        result = original_image.copy()
        if self.show_mask_heatmaps:
            return self.create_mask_montage(result, top_predictions)
        result = self.overlay_boxes(result, top_predictions, alpha=alpha, box_pixel=box_pixel,top_k=k)
        # result = self.overlay_entity_names(result, top_predictions, text_size=text_size, text_pixel=text_pixel, text_offset = text_offset, text_offset_original = text_offset_original)
        # TODO
        if show_score == 'True':
            result = self.overlay_scores(result, top_predictions)
    ...
    def compute_prediction(self, original_image, original_caption, custom_entity = None):
        ...
        # compute predictions
        with torch.no_grad():
            selection = self.model.getTaskIds(image_list)
            print(f'predicted selection {selection}')
            predictions = self.model(image_list, captions=[original_caption], positive_map=positive_map_label_to_token, task_id=selection)
    ...
    def _post_process_top_k(self, predictions, threshold=0.5, k=5):
        scores = predictions.get_field("scores")
        labels = predictions.get_field("labels").tolist()
        thresh = scores.clone()
        for i, lb in enumerate(labels):
    ...
    def overlay_boxes(self, image, predictions, alpha=0.5, box_pixel = 3, top_k=5):
        # TODO
        ...
        # TODO
        # colors = [(255,0,0), (255,215,0), (0,0,255), (0,255,0), (160,32,240)] BGR
        # colors = [(0,0,255), (0,215,255), (255,0,0), (0,255,0), (240, 32, 160)] # RGB
        # colors = [(255,255,255)]
        colors = ncolors(top_k)
        ...
        for box, color in zip(boxes, colors):
            ...
            new_image = cv2.rectangle(
                new_image, tuple(top_left), tuple(bottom_right), color, box_pixel)
