...
def build_refexp_evaluator(refexp_gt, iou_types, ids):
    from maskrcnn_benchmark.data.datasets.refexp import RefExpEvaluator
    evaluator = RefExpEvaluator(refexp_gt, iou_types, ids)
    return evaluator
...
def inference(
    ...
    elif "refexp" in cfg.DATASETS.TEST[0]:
        evaluator = build_refexp_evaluator(data_loader.dataset.coco, 'bbox', dataset.ids)
    else:
        evaluator = None
    ...
    results_dict = {}
    cpu_device = torch.device("cpu")
    logging.info(f"test sample number: {len(data_loader.dataset)}")
    if verbose:
            ...
            if cfg.TEST.USE_MULTISCALE:
                ...
                for query_i in range(query_time):
                    ...
                    if task == "detection":
                        ...
                        task_id = [t.get_field("task_id") for t in targets]
                    ...
                    if model.cfg.LPAI.INTERACT_TYPE != 'l2p':
                        selection = model.getTaskIds(images)
                    else:
                        selection = None
                    # print(selection, task_id[0])
                    total = total+1
                    if model.cfg.LPAI.INTERACT_TYPE != 'l2p':
                    ...
                    # flops, params = profile(model.to(device), inputs=(images,None, captions, positive_map_label_to_token,None, selection))
                    # logger.info(f'flops: {flops}, params: {params}')
                    # exit(0)
                    output = model(images, captions=captions, positive_map=positive_map_label_to_token, task_id=selection)
                    output = [o.to(cpu_device) for o in output]
                    ...
                    if "flickr" in cfg.DATASETS.TEST[0]:
                        ...
                        labels = output.extra_fields["labels"]
                        boxes = output.bbox
                        # if targets[0].extra_fields["image_id"].item() == 120669:
                        #     print(scores)
                        #     exit(1)
                        mdetr_style_output.append((targets[0].extra_fields["image_id"].item(),
                                                   {"scores": scores, "labels": labels, "boxes": boxes}))
                        mdetr_style_output_eval.update({targets[0].extra_fields["image_id"].item():{
                            "scores": scores,
                            "labels": labels,
                            "boxes": boxes
                        }})
                        ...
                        all_output.append(output)
        if visualizer is not None and i <=50:
            ...
            if "lvis" in cfg.DATASETS.TEST[0]:
                ...
                # TODO
                visualizer_input = all_output[0] # single image_visualize
            ...
            #     color=color,
            # )
            imshow(result, "./visualize/{}/img_{}.jpg".format(task_id[0].item(),captions[0]))
        
        if evaluator is not None:
            evaluator.update(mdetr_style_output_eval)
        else:
    ...
    if evaluator is not None:
        ...
        score = evaluator.summarize(task_id)
        # score = None
        # print(score)
        ...
        logger.info(f'task id prediction accuracy: {cor * 1.0 / total}')
        return score
