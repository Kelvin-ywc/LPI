...
def save_task_div(prefix, ids, task_id, is_train):
    train_name = "train" if is_train else "val"
    if not os.path.exists(prefix):
        os.mkdir(prefix)
    filename = f'{prefix}/GLIP_TASK_{task_id}_{train_name}'
    with open(filename, "w") as file:
        for item in ids:
            file.write(str(item) + "\n")

def read_list_from_file(prefix, task_id, is_train):
    train_name = "train" if is_train else "val"
    filename = f'{prefix}/GLIP_TASK_{task_id}_{train_name}'
...
class ModulatedDataset(torchvision.datasets.CocoDetection):
    def __init__(self,
        ...
        self.task_name = None
        if not transforms:
            self.transforms = [trans.Resize(320)]
        ids = []
        cats = []
        cat2scat = {}
        scat_id = {'person': 11, 'vehicle': 8, 'outdoor': 2, 'animal': 10, 'accessory': 4, 'sports': 1, 'kitchen': 6,
                   'food': 9, 'furniture': 7, 'electronic': 3, 'appliance': 0, 'indoor': 5}
        id_scat = {}
        for key in scat_id:
            id_scat[scat_id[key]] = key
        
        cnt = [0] * 12
        for key in self.coco.cats:
            cur_cat = self.coco.cats[key]
            cat2scat[cur_cat['id']] = scat_id[cur_cat['supercategory']]
        ...
        for tid in task_id:
            ...
            for img_id in self.ids:
                ...
                anno = self.coco.loadAnns(ann_ids)
                if has_valid_annotation(anno):
                    if cat2scat[anno[0]['category_id']] == tid:
                        cur_ids.append(img_id)

            ids.extend(cur_ids)

        self.ids = ids
        prefix = ''
        if '_refcoco_' in ann_file:
            self.dataset_name = 'refcoco'
            if 'train' in ann_file:
                prefix = './TASK_DIV/REFCOCO/train'
                self.task_name = 'train'
            if 'val' in ann_file:
                prefix = './TASK_DIV/REFCOCO/val'
                self.task_name = 'val'
            if 'testA' in ann_file:
                prefix = './TASK_DIV/REFCOCO/testA'
                self.task_name = 'testA'
            if 'testB' in ann_file:
                prefix = './TASK_DIV/REFCOCO/testB'
                self.task_name = 'testB'
        elif '_refcoco+_' in ann_file:
            self.dataset_name = 'refcoco+'
            if 'train' in ann_file:
                prefix = './TASK_DIV/REFCOCO+/train'
                self.task_name = 'train'
            ...
            if 'train' in ann_file:
                prefix = './TASK_DIV/REFCOCOg/train'
                self.task_name = 'train'
            if 'val' in ann_file:
                prefix = './TASK_DIV/REFCOCOg/val'
                self.task_name = 'val'
            if 'test' in ann_file:
                prefix = './TASK_DIV/REFCOCOg/test'
                self.task_name = 'test'
        # first time generate id

        # if is_train:
        #     save_task_div(prefix, self.ids, task_id[0], is_train)
        #     # print('train')
        # else:
        #     save_task_div(prefix, self.ids, task_id[-1], is_train)

        if is_train:
            list = read_list_from_file(prefix, task_id[0], is_train)
            assert len(self.ids) == len(list)
            for tmp_id in self.ids:
                assert tmp_id in list

            # save_task_div(self.ids, task_id[0],is_train)
        else:
            list = read_list_from_file(prefix, task_id[-1], is_train)
            assert len(self.ids) == len(list)
            for tmp_id in self.ids:
                assert tmp_id in list
            # save_task_div(self.ids, task_id[-1], is_train)

        logger = logging.getLogger("maskrcnn_benchmark")
        if is_train:
            logger.info(f'Training on task {task_id[0]}: {id_scat[task_id[0]]}, total training sample size: {len(self.ids)}')
        else:
            logger.info(f'Testing on task {task_id[-1]}: {id_scat[task_id[-1]]}, total testing sample size: {len(self.ids)}')

        self.cat2scat = cat2scat
        self.id_to_img_map = {k: v for k, v in enumerate(self.ids)}
        self._transforms = transforms
        self.max_query_len = max_query_len
    ...
    def __getitem__(self, idx):
        ...
        target.add_field(
            "task_id", torch.tensor([self.cat2scat[classes.item()]])
        )
